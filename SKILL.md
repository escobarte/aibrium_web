...
- **Right column (visual):** **FINAL — 2026-07-27. The hero visual is the Aibrium logo, not a photograph. There is no hero photo and none is coming; do not reintroduce a placeholder or a `hero.webp` image here.** A light **cream rounded card** (`bg-cream`, `rounded-xl`, `aspect-[7/5]`) with the logo (`/public/hero-logo.png`, 977×1000, `next/image`, `width`/`height` set so there is no layout shift, `priority`, alt `Aibrium Studio`) **centred inside it with generous padding** — a centred mark, not a panel-filling image; it must never touch the card edges. The logo artwork is black + gold, so it **must** stay on this light card — never place it directly on the ink background. Soft gold bloom behind the card. *(The logo PNG currently ships with an opaque white background, so it reads as a white block on the cream fill until a transparent-alpha version replaces it.)*
