...
- **Right column (visual):** **FINAL — 2026-07-27. The hero visual is the Aibrium logo, not a photograph. There is no hero photo and none is coming; do not reintroduce a placeholder or a `hero.webp` image here.** A light **cream rounded card** (`bg-cream`, `rounded-xl`, `aspect-[7/5]`) with the logo centred inside it, **desktop only**. Use the **transparent** file `/public/hero-logo-transparent.png` (RGBA, square 4167×4167) — `next/image`, `width={800} height={800}` (square, so no layout shift), `priority`, alt `Aibrium Studio`, `object-contain`. **Never** the old opaque `hero-logo.png`: it has no alpha and paints its own white rectangle over the cream, giving a visible box-inside-a-box. There must be **no background, fill or wrapper behind the logo** — the artwork sits straight on the cream card. Even `p-28` padding centres it as a mark with breathing room, never touching the card edges and never panel-filling. The artwork is black + gold, so it **must** stay on this light card — never place it directly on the ink background. Soft gold bloom behind the card.
  - **Why `p-28` specifically — do not shrink it:** the two floating cards below have **fixed** heights (75px / 72.5px) at a fixed `top-10` / `bottom-10` inset, while this card is **fluid** (456px→544px wide from `lg` to 1200px+), so the clear band between them is only `cardH − 227`. `p-28` caps the logo box at `cardH − 224`, which keeps the artwork clear of both cards across the whole desktop range (measured from the file's alpha channel — artwork occupies the middle 62.9%×64.6% of the square frame, 15.8% clear above it and 19.6% below: **13px / 19px** clearance at `lg`, **23px / 32px** at 1200px+). If the mark needs to be bigger, move the floating cards out of the band — do not reduce this padding, or the wordmark gets clipped again.
...
- **Bottom line:** `© 2026 Aibrium Studio` — exactly this, nothing else. *(Simplified 2026-07-28: the legal entity, the `t/a` phrase, the city/country and the Privacy Policy link were all removed from the footer. Do not restore them.)* The `/privacy` page and route still exist and still carry the legal entity in their own copy — they are simply no longer linked from the footer.
