...
- **Right column (visual):** **FINAL — 2026-07-28. The hero visual is the Aibrium logo, not a photograph. There is no hero photo and none is coming; do not reintroduce a placeholder or a `hero.webp` image here.** A light **cream rounded card** (`bg-cream`, `rounded-xl`, `aspect-[7/5]`) with the **horizontal logo lockup** (logomark left, `AIBRIUM` / `STUDIO` right) centred inside it, **desktop only**. Use `/public/hero-logo-wide.png` — `next/image`, `width={850} height={400}` (**2.125:1, the real artwork ratio — never 1:1**), `priority`, alt `Aibrium Studio`, `object-contain`, even `p-32` padding. There must be **no background, fill or wrapper behind the logo** — the transparent artwork sits straight on the cream card. It is black + gold, so it **must** stay on this light card — never place it directly on the ink background. Soft gold bloom behind the card. *(Do not use `hero-logo.png` — no alpha, it paints its own white rectangle over the cream. Do not use `hero-logo-transparent_old.png` — the superseded vertical lockup, which collided with the floating cards.)*
  - **The file must stay trimmed.** `hero-logo-wide.png` is the designer's `hero-logo-transparent.png` losslessly cropped to its content bbox (**x 525–3735, y 1327–2838** of the 4167² canvas) → **3211×1512, aspect 2.1237**. That canvas is **97.4% transparent margin** (≈32% dead space above *and* below the artwork); pointed at it untrimmed, the dead space counts as layout box and the lockup renders tiny inside a big empty square. If the logo is ever redrawn, **re-trim to the content bbox** rather than compensating in CSS.
  - **Why `p-32` specifically — do not shrink it:** the two floating cards below have **fixed** heights (75px / 72.5px) at a fixed `top-10` / `bottom-10` inset, while this card is **fluid** (456px→544px wide from `lg` to 1200px+), so the clear band between them is only `cardH − 227`. Because the lockup is wide and the two cards sit at **opposite corners**, staying inside that band clears **both** of them at any width. `p-32` caps the logo at `cardH − 256` tall: **148.0×69.7** at `lg`, **281.6×132.6** at 1200px+ (51.8% of the card width), each clearing card A by **13px** and card B by **15.5px**. If the mark needs to be bigger, move the floating cards out of the band — do not reduce this padding, or the lockup gets clipped again.
